...
import com.goldbookapp.inapplock.encryption.Encryptor;
import com.goldbookapp.inapplock.enums.Algorithm;
...
import static com.goldbookapp.utils.Constants.PASSWORD_PREFERENCE_KEY;

public class AppLockImpl<T extends AppLockActivityNew> extends AppLock implements LifeCycleInterface {
    ...
    /**
     * The {@link SharedPreferences} key used to store whether the user has backed out of the {@link AppLockActivityNew}
     */
     ...
     * @param activityClass The activity extending {@link AppLockActivityNew}
     * @return The instance.
     */
    public static AppLockImpl getInstance(Context context, Class<? extends AppLockActivityNew> activityClass) {
    ...
    public boolean setPasscode(String passcode) {
        ...
        if (passcode == null) {
           ...
           /* editor.putString(PINCODE,passcode);
            editor.apply();*/
//            passcode = salt + passcode + salt;
    ...
    @Override
    public String getPasscode(String passcode){
        setAlgorithm(Algorithm.SHA256);
        passcode = Encryptor.getSHA(passcode, Algorithm.SHA256);
        return passcode;
    }
    ...
    public boolean shouldLockSceen(Activity activity) {
        Log.d(TAG, "Goldbook shouldLockSceen() called");
        ...
        // already unlock
        if (activity instanceof AppLockActivityNew) {
            AppLockActivityNew ala = (AppLockActivityNew) activity;
    ...
    public void onActivityPaused(Activity activity) {
        ...
        if (!shouldLockSceen(activity) && !(activity instanceof AppLockActivityNew)) {
            setLastActiveMillis();
        }
    }
    ...
    public void onActivityUserInteraction(Activity activity) {
        if (onlyBackgroundTimeout() && !shouldLockSceen(activity) && !(activity instanceof AppLockActivityNew)) {
            setLastActiveMillis();
        }
    }
