...
class NewPurchaseViewModel(private val apiHelper: ApiHelper) : ViewModel() {
    ...
    /*fun getSearchVendor(
        ...
        company_id: String?,
        search: String?,
        offset: String?,transaction_type: String?
    ) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))
        try {
            emit(
                Resource.success(
                    data = apiHelper.getSearchVendor(
                        token, company_id, search,
                        offset,transaction_type
                    )
                )
            )
    ...
    fun getSearchContacts(
        ...
        company_id: String?,
        search: String?,
        offset: String?, transaction_id: String?
    ) = liveData(Dispatchers.IO) {
        emit(Resource.loading(data = null))
        try {
            emit(
    ...
    fun getCalculateItemPurchase(
        ...
        try {
            emit(
                Resource.success(
                    data = apiHelper.getCalculateItemPurchase(
                        token,
                        calculate,
                        contact_id,
                        transaction_id,
    ...
    fun getPurchaseInvoiceNoFromApi(
        ...
        try {
            emit(
                Resource.success(
                    data = apiHelper.getPurchaseInvoiceNoFromApi(
                        token,
                        invoice_date,
                        transaction_id
                    )
