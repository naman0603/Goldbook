...
import com.goldbookapp.databinding.FragmentTransactionBinding
...
class TransactionsFragment(var customerDetailModel: CustomerDetailModel, var contxt: Context) :
    Fragment() {
    ...
    override fun onResume() {
        ...
        getTransactionHistoryDetails(
            ...
            customer_id, currentPage,start_date,end_date
        )
    ...
    fun setupUIandListner(root: View) {
        ...
        if (customerDetailModel.customers != null) {
            customer_id = customerDetailModel.customers!!.id.toString()
        }
        ...
        binding.monthSpinner.text =
            "${CommonUtils.getMonthByNumber(cal[Calendar.MONTH] + 1)} - $selectedYearGlobal"
        sortOrder = "desc"
        //getTransactionHistoryDetails( loginModel?.data?.bearer_access_token,customer_id,selectedMonth,sortOrder)
        //setUpMonthSpinner()
        setUpSortSpinner()
        setupFiscalYearDialog()
        getFiscalYear()

        binding.monthSpinner.clickWithDebounce {
            setupMonthYearDialog(cal)
        }

        binding.linearFiscalYearTransaction.clickWithDebounce {
        ...
        binding.rvTransactions.addOnScrollListener(object :
            PaginationListener(binding.rvTransactions.layoutManager as LinearLayoutManager) {
    ...
    fun compareDates(d1: String?, d2: String?) {
        try {
            ...
            binding.tvtransactionFiscalYear.setText(
    ...
    private fun openFiscalYearDialog() {
        ...
        dialog1.tvFiscalSave.clickWithDebounce {
            dialog1.dismiss()
            binding.tvtransactionFiscalYear.setText("F.Y.: " + start_date + " to " + end_date)
            currentPage =  1
            getTransactionHistoryDetails(
                loginModel.data?.bearer_access_token,
                customer_id, currentPage,start_date,end_date
...
//        val c = Calendar.getInstance()
                ...
                when (isFromDate) {
                    ...
                    true -> {
                        ...
                        if (parse.after(sdf.parse(dialog1.txtFromDate.text.toString()))) {
                            ...
                            end_date = SimpleDateFormat("dd-MMM-yyyy").format(dialog1.txtToDate.text.toString())
                        }
    ...
    private fun setupMonthYearDialog(today: Calendar) {
        ...
        if (selectedYr == -1) {
            ...
            builder.setActivatedMonth(selectedMon)
                .setActivatedYear(selectedYr)
    ...
    private fun setUpSortSpinner() {
        ...
        // var aa = ArrayAdapter(this, android.R.layout.simple_spinner_item, monthArrayList)
    ...
    fun getTransactionHistoryDetails(
        ...
        if (NetworkUtils.isConnected()) {
            ...
            transactionViewModel.transactionHistory(token, contact_id, current_page,date_range_from,date_range_to)
                        ...
                        when (resource.status) {
                            Status.SUCCESS -> {
                                if (it.data?.status == true) {
                                    ...
                                    {
                                       /* binding.tvNoRecordTransaction.text =
                                            "No Transaction for the selected month"*/
                                        binding.rvTransactions.visibility = View.GONE
                                        binding.transationTitles.visibility= View.GONE
                                        binding.tvNoRecordTransaction.visibility = View.VISIBLE

                                    } else {
                                        binding.rvTransactions.visibility = View.VISIBLE
                                        binding.transationTitles.visibility= View.VISIBLE
                                        binding.tvNoRecordTransaction.visibility = View.GONE
                                    }
                                    isFirstime = true
    ...
    private fun setTransactionHistoryDetails(
       ...
       /* binding.tvTotalEntrieTransaction.text =
        ...
        when (transactionHistoryModel.transactionsum?.debit_sum_fine_wt.toString()) {
            "0.000" -> {
                binding.tvTotalOutwardTransactionOne.text =
                    transactionHistoryModel.transactionsum?.debit_sum_fine_wt.toString()
                binding.tvTotalOutwardTransactionOne.setTextColor(
        ...
        when (transactionHistoryModel.transactionsum?.debit_sum_amount.toString()) {
            "0.00" -> {
                binding.tvTotalOutwardTransactionTwo.text =
                    transactionHistoryModel.transactionsum?.debit_sum_amount.toString()
                binding.tvTotalOutwardTransactionTwo.setTextColor(
                    ContextCompat.getColor(
                        contxt,
                        R.color.header_black_text
                    )
        ...
        when (transactionHistoryModel.transactionsum?.credit_sum_fine_wt.toString()) {
            "0.000" -> {
                binding.tvTotalInwardTransactionOne.text =
                    transactionHistoryModel.transactionsum?.credit_sum_fine_wt.toString()
                binding.tvTotalInwardTransactionOne.setTextColor(
                    ContextCompat.getColor(
                        contxt,
                        R.color.header_black_text
                    )
        ...
        when (transactionHistoryModel.transactionsum?.credit_sum_amount.toString()) {
            "0.00" -> {
                binding.tvTotalInwardTransactionTwo.text =
                    transactionHistoryModel.transactionsum?.credit_sum_amount.toString()
                binding.tvTotalInwardTransactionTwo.setTextColor(
                    ContextCompat.getColor(
                        contxt,
                        R.color.header_black_text
                    )
