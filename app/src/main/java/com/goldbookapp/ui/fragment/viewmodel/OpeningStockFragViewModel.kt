...
class OpeningStockFragViewModel(private val apiHelper: ApiHelper) : ViewModel() {

    private val _text = MutableLiveData<String>().apply {
        value = "This is share Fragment"
    }
    val text: LiveData<String> = _text
    fun searchListOpeningStock(token: String?, company_id: String?, current_page: Int?, name: String?, sort_by_column: String?,
                        sort_type: String?,date_range_from: String?,date_range_to: String?) = liveData(Dispatchers.IO){
        emit(Resource.loading(data = null))
        try {
            emit(Resource.success(data = apiHelper.searchListOpeningStock(token, company_id, current_page,
                name, sort_by_column, sort_type,date_range_from,date_range_to)))
