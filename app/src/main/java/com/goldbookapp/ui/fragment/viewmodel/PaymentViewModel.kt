...
class PaymentViewModel (private val apiHelper: ApiHelper) : ViewModel() {
    ...
    fun searchListPayment(token: String?, current_page: Int?, name: String?, sort_by_column: String?,
                           sort_type: String?,date_range_from: String?,date_range_to: String?) = liveData(Dispatchers.IO){
        emit(Resource.loading(data = null))
        try {
            emit(Resource.success(data = apiHelper.searchListPayment(token, current_page,name, sort_by_column,
