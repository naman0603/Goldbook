...
) : RecyclerView.Adapter<NewPurchaseItemAdapter.DataViewHolder>() {
    ...
    class DataViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
                        ...
                        when(addinvoice_item_rowModel.item_rate_on){
                            "fine"->{
                                ...
                                val fineWeight: BigDecimal =
                                    addinvoice_item_rowModel.item_fine_wt.toBigDecimal()
                                val totalAmt: String =
                                    ((goldRate.setScale(3)
        ...
        fun DeleteConfirmDialog(context: Context, isfromNewInvoice: Boolean) {
            ...
            val itemDeleteClick = { dialog: DialogInterface, which: Int ->
                if (isfromNewInvoice)
                    (context as NewPurchaseActivity).removeItem(position)
                else {
                    (context as NewPurchaseActivity).removeItem(position)
                }
